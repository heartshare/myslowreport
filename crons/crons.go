...
func createSlowInfo(items []models.Item, p models.Project, tableId int, mi models.MaxItem) string {
	...
	info += `<input type="button" value="显示趋势/隐藏趋势" onClick="showTrend('{{.canvasDiv}}')" />`
	info += `<div id="{{.canvasDiv}}" style="width:70%;display:none;">`
	info = strings.Replace(info, "{{.canvasDiv}}", fmt.Sprintf("canvasDiv%d", tableId), -1)
	info += `<canvas id="{{.canvas}}"></canvas>`
	info = strings.Replace(info, "{{.canvas}}", fmt.Sprintf("canvas%d", tableId), -1)
	info += `<script>`
	info += `var l = {
			labels: [{{.DateLabels}}],
			datasets: [{
	...
	info = strings.Replace(info, "{{.Func}}", fmt.Sprintf("canvasFunc%d", tableId), -1)
	info = strings.Replace(info, "{{.canvas}}", fmt.Sprintf("canvas%d", tableId), -1)
	info += `</script>`
	info += `</div>`
